...
function FiaAnswer({ answer, source, resultCount, className }: FiaAnswerProps) {
  return (
    ...
    >
      {/* Hanging indent: text-indent reserves the icon's gutter on line 1 only, so
          wrapped lines fall back to the block's own left edge (no icon offset). */}
      <div className="relative w-full">
        <FiaSilcrow className="absolute top-[4px] left-0 text-[color:var(--tw-raw-fia-200)]" />
        <p
          className={cn(
            paragraphStyle,
            'w-full break-words indent-[calc(var(--icon-sm)_+_var(--spacing-2xs))] text-[color:var(--theme-alpha-black-switch-70)]',
          )}
        >
          {answer}
        </p>
