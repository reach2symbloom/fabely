/**
 * Fabely Sidebar — shadcn Sidebar composition restyled from Figma Sidebar.
 *
 * Figma: Sidebar page (`842:51929`) — Sidebar Item (1st/2nd level), Group Label,
 * Badge, Collapsed; example shell ~240 (`15rem`). Public API matches
 * [shadcn Sidebar](https://ui.shadcn.com/docs/components/base/sidebar).
 * Vendor (`src/components/ui/sidebar.tsx`) stays untouched.
 */
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          "relative w-(--sidebar-width) bg-transparent transition-[width] duration-[var(--duration-fast)] ease-linear",
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex w-full flex-1 flex-col bg-background md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-[length:var(--rounded-xl)] md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2",
        className
      )}
      {...props}
    />
...
function SidebarSeparator({
  ...
  return (
    <Separator
      ...
      className={cn(
        'mx-[var(--spacing-xs)] w-auto bg-sidebar-border',
        className,
      )}
      {...props}
    />
...
function SidebarGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="group"
      className={cn(
        'relative flex w-full min-w-0 flex-col p-[var(--spacing-xs)]',
        className,
      )}
      {...props}
    />
...
function SidebarGroupLabel({
  ...
  return useRender({
    ...
    props: mergeProps<"div">(
      {
        className: cn(
          'flex h-[length:var(--spacing-2xl)] shrink-0 items-center rounded-[length:var(--rounded-md)] px-[var(--spacing-sm)] py-[var(--spacing-xs)]',
...
const sidebarMenuButtonVariants = cva(
  [
    'peer/menu-button group/menu-button flex w-full items-center gap-[var(--spacing-xs)] overflow-hidden',
    'rounded-[length:var(--rounded-md)] px-[var(--spacing-sm)] py-[var(--spacing-2xs)]',
    'text-left font-[family-name:var(--text-paragraph-small-regular-font-family)]',
    'text-[length:var(--text-paragraph-small-regular-font-size)] leading-[var(--text-paragraph-small-regular-line-height)]',
    ...
    '[&_svg]:size-[length:var(--icon-sm)] [&_svg]:shrink-0 [&>span:last-child]:truncate',
  ].join(' '),
  {
    variants: {
      variant: {
...
function SidebarMenuSkeleton({
  ...
  return (
    <div
      ...
      className={cn("flex h-8 items-center gap-2 rounded-[length:var(--rounded-md)] px-2", className)}
      {...props}
    >
      {showIcon && (
        <Skeleton
          className="size-[length:var(--icon-sm)] rounded-[length:var(--rounded-md)]"
    ...
    props: mergeProps<"a">(
      {
        className: cn(
          [
            ...
            'disabled:pointer-events-none disabled:opacity-50',
            'aria-disabled:pointer-events-none aria-disabled:opacity-50',
            'data-[size=md]:text-[length:var(--text-paragraph-small-regular-font-size)]',
            'data-[size=sm]:text-[length:var(--text-paragraph-mini-regular-font-size)]',
            'data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground',
            '[&>span:last-child]:truncate [&>svg]:size-[length:var(--icon-sm)] [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground',
          ].join(' '),
          className,
        ),
      },
