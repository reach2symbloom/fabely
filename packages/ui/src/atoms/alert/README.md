...
The Fabely Alert atom — wraps the upstream shadcn Alert primitive (`src/components/ui/alert.tsx`) to match this design system's own Figma Alert component (node `58:5416`, "Fabely Design System" file: `gV94L0qCmvwQkddNbEktry`), inspected directly via the Figma MCP connection.
...
| Icon shown/hidden | Not a prop for any of the 4 types — derived from whether an icon element (any non-`AlertTitle`/`AlertDescription` child) is present, `success` included. Figma's own Success schema has no icon slot at all (its checkmark instance there is unconditional), but this atom deliberately derives its visibility from composition anyway, matching the other 3 types, rather than forcing it permanently on. Whichever icon element is passed for `type="success"` is ignored in favor of a fixed checkmark — only its *presence* is used as the show/hide signal, not its identity. |
| `Flip Icon` | Not implemented. |
| `Show Button` | Not implemented — no `AlertAction` in this atom. |
...
Vendor's own `variant` prop (`"default" | "destructive"`) is still not exposed on this atom's `Alert` — `type` is the equivalent surface now, with its own 4 values (a superset of what `variant` could express).
...
- **`error`** — background is 8% of `--tw-raw-error-400`; border is 32% of `--tw-raw-error-300`. Neither percentage matches an already-declared step (`--tw-raw-error-ghost` is a *different*, already-declared 12% step). The ring, unlike background/border, *is* an existing Foundation token used as-is: Figma's own inspected effect here is "focus ring error," which resolves to exactly `--ring-error`'s already-declared, already-switching value (light: `--tw-raw-error-100`, dark: `--tw-raw-error-ghost`). Title text binds to the raw, non-switching `--tw-raw-error-50` — see "Known limitations" below.
...
The icon's color is intentionally *independent* of the title-text color documented above — in Figma every type's icon reads as a distinct semantic accent, not the same (sometimes near-white/pale) shade as its own title. This atom overrides vendor's own `[&>svg]:text-current` (which would otherwise make the icon inherit whatever `text-*` color is on the Alert root, i.e. the title's own color) with a per-type `[&>svg]:text-*` rule of the same selector shape, so `tailwind-merge` reliably drops vendor's default:
