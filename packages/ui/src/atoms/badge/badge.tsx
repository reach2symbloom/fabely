/**
 * Fabely Badge atom — wraps the upstream shadcn Badge primitive
...
function toVendorVariant(variant: BadgeVariant): VendorVariant {
  switch (variant) {
     ...
     * like success/alert; visible styles come from this atom. */
    case 'destructive':
    case 'success':
    case 'alert':
      return 'default';
...
const badgeVariants = cva(
  [
     ...
     * (--tw-raw-spacing-3). Large scales with Paragraph Small. data-icon
     * attrs support the shadcn docs' inline-start / inline-end composition
     * without adding new product props. */
    ...
    variants: {
      ...
      size: {
        ...
        large: [
          ...
          /* TODO: Figma Large + icon frames weren't exported with an explicit
           * icon size in the design-context payload — 14px matches Paragraph
           * Small's font-size rather than a confirmed Figma icon token. */
          '[&>svg]:size-[14px]',
        ],
      },
      roundness: {
